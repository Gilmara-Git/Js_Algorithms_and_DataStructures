...
class Graph {
  ...
  DFSIterative(vertex) {
    let result = [];
    let visited = {};
    let stackRepresentedInArray = [];
    let mapPoint;

    stackRepresentedInArray.push(vertex);
    visited[vertex] = "discovered";

    while(stackRepresentedInArray.length) {
      mapPoint = stackRepresentedInArray.pop();
      result.push(mapPoint);

      this.adjacencyList[mapPoint].forEach((connection) => {
        if (!visited[connection]) {
            visited[connection] = "discovered";
          stackRepresentedInArray.push(connection);
        }
      });
    }
